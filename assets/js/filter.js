(function() {
  ...
  if (projectControls) {
    ...
    config = {
      ...
      sectionHashKey: 'cat',
      primaryAttr: 'data-tags',
      primaryMultiValue: true,
      primaryHashKey: 'tag',
      primaryPlaceholder: 'Add collection tag...',
      searchAttr: 'data-search',
      searchHashKey: 'q',
  ...
  var sortValue = config.sortDefault || '';
  var sortSelect = null;
  ...
  function initProjectDescriptions() {
    ...
    for (var i = 0; i < rows.length; i++) {
      rows[i].classList.add('project-descriptions-ready');
      (function(row) {
        var toggle = row.querySelector('.project-desc-toggle');
  ...
  function buildFilterBar() {
    ...
    if (config.sortOptions) {
      ...
      for (var i = 0; i < config.sortOptions.length; i++) {
        var sortOption = document.createElement('option');
        sortOption.value = config.sortOptions[i].value;
        sortOption.textContent = 'Sort: ' + config.sortOptions[i].label;
        sortOption.selected = sortOption.value === sortValue;
  ...
  function updateHash() {
    ...
    if (config.sortHashKey && sortValue && sortValue !== config.sortDefault) {
      parts.push(config.sortHashKey + '=' + encodeURIComponent(sortValue));
    }
  ...
  function parseHashFilters() {
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      } else if (config.sortHashKey && params[i].indexOf(config.sortHashKey + '=') === 0) {
        sortValue = decodeURIComponent(params[i].slice(config.sortHashKey.length + 1));
      }
    }
  ...
  function sortLibraryItems() {
    ...
    var lists = config.list.querySelectorAll('.library-list');
    for (var i = 0; i < lists.length; i++) {
      var items = Array.prototype.slice.call(lists[i].querySelectorAll('.library-item'));
      items.sort(function(a, b) {
        var titleA = a.getAttribute('data-title') || '';
        var titleB = b.getAttribute('data-title') || '';
        if (sortValue === 'author') {
          var authorA = a.getAttribute('data-author') || titleA;
          var authorB = b.getAttribute('data-author') || titleB;
        ...
        if (sortValue === 'newest' || sortValue === 'oldest') {
          ...
          if (yearA === 0 || yearB === 0) {
            if (yearA === yearB) return titleA.localeCompare(titleB);
            return yearA === 0 ? 1 : -1;
          }
